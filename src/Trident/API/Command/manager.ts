...
export declare type CommandArgDefinition = {
  name: string;
...
export class CommandArg {
   ...
   */
  static literal(name: string, required: boolean, values: string[], subArgs?: Record<string, CommandArg[]>) {
   ...
   * @remarks Use ```interaction.getString(argName)``` to get this argument
   */
  static string(name: string, required: boolean, minLength?: number, maxLength?: number, matches?: RegExp, subArgs?: Record<string, CommandArg[]>) {
    return new CommandArg({
      name, 
      type: 'string', 
      required, 
   ...
   */
  static number(name: string, required: boolean, min?: number, max?: number, float?: boolean, subArgs?: Record<string, CommandArg[]>) {
    return new CommandArg({
       name, 
       type: 'number', 
       required, 
   ...
   */
  static boolean(name: string, required: boolean, subArgs?: Record<string, CommandArg[]>) {
    return new CommandArg({
       name, 
       type: 'boolean', 
       required, 
   ...
   */
  static player(name: string, required: boolean, self?: boolean, subArgs?: Record<string, CommandArg[]>) {
    return new CommandArg({
      name, 
      type: 'player', 
      required,
   ...
   */
  static time(name: string, required: boolean, subArgs?: Record<string, CommandArg[]>) {
    return new CommandArg({
       name, 
       type: 'time', 
       required, 
...
export class CommandInteraction {
  ...
  constructor(args: Record<string, any>, ctx: Player) {
    this.args = args;
    this.ctx = ctx
  }
   ...
   * @returns 
   */
  getString(argName: string): string {
    const value = this.args[argName];
   ...
   * @returns 
   */
  getInteger(argName: string): number {
    const value = this.args[argName];
    if (typeof value !== "number") throw new Error(`Argument "${argName}" is not of type number.`);

    if (!Number.isInteger(value)) throw new Error(`Argument "${argName}" is not an integer.`);
   ...
   * @returns 
   */
  getFloat(argName: string): number {
    const value = this.args[argName];
    if (typeof value !== "number") throw new Error(`Argument "${argName}" is not of type number.`);

    if (Number.isInteger(value)) throw new Error(`Argument "${argName}" is not a float.`);
   ...
   * @returns 
   */
  getPlayer(argName: string): Player {
    const value = this.args[argName];
   ...
   * @returns 
   */
  getTime(argName: string): number {
    const value = this.args[argName];
    if (!value?.match(timeRegex)?.length) throw new Error(`Argument "${argName}" is not of type time.`);
   ...
   * @returns 
   */
  getPosition(argName: string): Vector3 {
    ...
    if (!vector?.x || !vector?.y || !vector?.z || isNaN(vector.x) || isNaN(vector.y) || isNaN(vector.z)) throw new Error(`Argument "${argName}" is not of type position.`);
    return vector
  }
...
class CommandHandler {
   ...
   * @param sender 
   */
  executeCommand(commandLine: string, sender: Player): void {
    try {
      const { name, rawArgs: rawArguments } = parseCommandLine(commandLine);
      let command = this.commands.get(name);
      const commands = [...this.all()]
      if (!command) command = commands.find(command => command?.aliases?.length && command.aliases.includes(name))
      ...
      function parseArgs(args: CommandArgDefinition[], rawArgs: string[], handler: CommandHandler) {
      ...
      if (command?.requires && !command.requires(sender)) throw new Error(`Unknown command: ${command.name}. Please check that the command exists and that you have permission to use it.`)
   ...
   * @returns 
   */
  private parseArg(value: string, arg: CommandArgDefinition, ctx: Player): any {
    const { type } = arg
    switch (type) {
      case "string":
        return value;
      case "number":
        ...
        if (isNaN(num)) throw new Error(`Argument ${arg.name} must be a number`);

        if (arg?.min && num < arg.min) throw new Error(`Argument ${arg.name} requires a number greather than or equal to ${arg.min}`);
        if (arg?.max && num > arg.max) throw new Error(`Argument ${arg.name} requires a number less than or equal to ${arg.max}`);

        if (!arg?.float && !Number.isInteger(num)) throw new Error(`Argument ${arg.name} requires an integer number`);
        return num;
      case "boolean":
        ...
        return value === "true";
      case "player":
        if (value === '@s' || value === '@p') value = ctx.name
        ...
        if (value === ctx.name && !arg?.self) throw Error(`Argument ${arg.name} requires a player that is not yourself`)

        const player = this.findPlayerByName(value)
        
        if (!player) throw Error(`Argument of type player must be an online player`)
        return player
      case 'literal':
        ...
        return value
      case 'time':
        if (!value?.match(timeRegex)?.length) throw new Error(`Argument ${arg.name} must be of type time`);
 ...
 * @returns 
 */
function parseCommandLine(commandLine: string): { name: string; rawArgs: string[] } {
